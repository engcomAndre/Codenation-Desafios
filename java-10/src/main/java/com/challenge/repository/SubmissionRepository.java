...
public interface SubmissionRepository extends BaseRepository<Submission, Long> {
    //Using SQL Querys
    @Query(value = SubmissionQuerys.SELECT.BY_CHALLENGE_ID, nativeQuery = true)
    BigDecimal findHigherScoreByChallengeId(@Param(value = "challengeId") Long challengeId);

    @Query(value = SubmissionQuerys.SELECT.BY_CHALLENGE_ID_AND_ACCELERATION_ID, nativeQuery = true)
